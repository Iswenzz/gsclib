...
PCRE2_VALUES* PCRE2_Match(PCRE2_SPTR subject, PCRE2_SPTR regex)
{
	...
	for (int i = 0; i < values->count; i++)
	{
		...
		{
			PCRE2_AddValue(&values->results[i], values->results[i].string, strlen(values->results[i].string), matches);
		}
	}
...
PCRE2_VALUES* PCRE2_Split(PCRE2_SPTR subject, PCRE2_SPTR regex)
{
	...
	for (int i = 0; i < values->count; i++)
	{
		if (!values->results[i].matched)
		{
			PCRE2_AddValue(&values->results[i], values->results[i].string, strlen(values->results[i].string), splits);
		}
	}
	PCRE2_FreeValues(values);
	return splits;
